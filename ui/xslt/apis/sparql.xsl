...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	...
	<xsl:param name="template">
		<xsl:choose>
			<xsl:when test="$api='avgAxis' or $api='avgDiameter' or $api='avgWeight'">avgMeasurement</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:when test="$template = 'avgMeasurement'">
				<xsl:call-template name="avgMeasurement">
					<xsl:with-param name="measurement" select="lower-case(substring-after($api, 'avg'))"/>
				</xsl:call-template>
			</xsl:when>
