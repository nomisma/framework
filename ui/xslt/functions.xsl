...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:nomisma="http://nomisma.org/" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0"
	...
	<xsl:function name="nomisma:expandDatetoDateTime">
		<xsl:param name="date"/>
		...
		<!--temporarily disable adding 1 year to BCE dates -->
		<!-- <xsl:value-of select="
		...
		<xsl:choose>
			<xsl:when test="substring($date, 1, 1) = '-'">
				<xsl:choose>
					...
					<xsl:when test="$date castable as xs:date">
						<xsl:value-of
							select="concat(xs:date($date), $time)"
						/>
					</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="$curie = 'nmo:DepositionType'">Deposition Type</xsl:when>
