...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:res="http://www.w3.org/2005/sparql-results#" exclude-result-prefixes="#all"
	...
	<xsl:template match="content" mode="json"> { "id": "<xsl:value-of select="identifier"/>", <xsl:apply-templates select="res:sparql[1]/res:results" mode="counts"/>
			...
			<xsl:when test="res:result/res:binding[@name='type']/res:uri='http://nomisma.org/ontology#NumismaticObject'">
				<xsl:choose>
					<xsl:when test="$format='json'">"object-count":<xsl:value-of
							select="res:result[res:binding[@name='type']/res:uri='http://nomisma.org/ontology#NumismaticObject']/res:binding[@name='count']/res:literal"/>,</xsl:when>
					<xsl:otherwise>
						<object-count>
							<xsl:value-of select="res:result[res:binding[@name='type']/res:uri='http://nomisma.org/ontology#NumismaticObject']/res:binding[@name='count']/res:literal"/>
						</object-count>
					</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:when test="res:result/res:binding[@name='type']/res:uri='http://nomisma.org/ontology#Hoard'">
				<xsl:choose>
					<xsl:when test="$format='json'">"hoard-count" : <xsl:value-of
							select="res:result[res:binding[@name='type']/res:uri='http://nomisma.org/ontology#Hoard']/res:binding[@name='count']/res:literal"/></xsl:when>
					<xsl:otherwise>
						<hoard-count>
							<xsl:value-of select="res:result[res:binding[@name='type']/res:uri='http://nomisma.org/ontology#Hoard']/res:binding[@name='count']/res:literal"/>
						</hoard-count>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template match="res:result" mode="xml">
		<object>
			<xsl:attribute name="collection" select="res:binding[@name='collection']/res:literal"/>
	...
	<xsl:template match="res:binding" mode="json">
		<xsl:text>"</xsl:text>
		<xsl:value-of select="if (@name = 'object') then 'uri' else @name"/>
		<xsl:text>":"</xsl:text>
		<xsl:value-of select="*"/>
		<xsl:text>"</xsl:text>
		<xsl:if test="not(position() = last())">
