...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:res="http://www.w3.org/2005/sparql-results#"
			...
			<examples>
				<_array>
					<xsl:for-each select="doc('input:config-xml')//dieStudy_formulas/formula">
						<_>
							<xsl:value-of select="concat($url, 'apis/dieCounts/', id, '?dieStudy=', encode-for-uri($dieStudy), '&amp;type=', encode-for-uri($type))"/>
						</_>
					</xsl:for-each>
				</_array>
			</examples>
	...
	<xsl:template match="formula">
		<_object>
			...
			<url>
				<xsl:value-of select="concat($url, 'apis/dieCounts/', id)"/>
			</url>
		</_object>
	</xsl:template>
