			...
			<td>
				...
				<dl class="dl-horizontal">
					...
					<xsl:if
						...
						<dt>Identifier</dt>
						<dd>
							<xsl:for-each select="tokenize(res:binding[@name = 'identifiers']/res:literal, '\|\|')">
								<xsl:choose>
					...
					<xsl:if test="string(res:binding[@name = 'types']/res:literal)">
						<xsl:variable name="typeURIs" select="distinct-values(tokenize(res:binding[@name = 'types']/res:literal, '\|'))"/>
						<xsl:variable name="typeTitles" select="distinct-values(tokenize(res:binding[@name = 'typeLabels']/res:literal, '\|'))"/>
