...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:gml="http://www.opengis.net/gml" xmlns:skos="http://www.w3.org/2008/05/skos#"
	...
	exclude-result-prefixes="xs gml exsl skos rdf xlink nuds nh" xmlns="http://earth.google.com/kml/2.0" version="2.0">
	<xsl:output method="xml" encoding="UTF-8"/>
	
	<xsl:variable name="geonames-url">
		<xsl:text>http://api.geonames.org</xsl:text>
	</xsl:variable>
	<xsl:variable name="geonames_api_key">anscoins</xsl:variable>
	...
	<xsl:template match="nh:nudsHoard">
		<xsl:for-each select="nh:descMeta/nh:hoardDesc/nh:findspot/nh:geogname[@xlink:role='findspot'][string(@xlink:href)]">
	...
	<xsl:template match="nuds:typeDesc">
		...
		<xsl:for-each select="exsl:node-set($typeDesc)/descendant::nuds:geogname[@xlink:role='mint'][string(@xlink:href)]">
			<Placemark id="{@xlink:href}">
				<name>
					<xsl:value-of select="."/>
				</name>
